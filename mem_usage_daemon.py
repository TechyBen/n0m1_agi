#!/usr/bin/env python3
"""Daemon that logs memory usage percent to the database."""
import time
import os
import sqlite3
import argparse
...
def main_loop(run_type):
    ...
    while True:
        ...
        cursor.execute(
            f"INSERT INTO {TABLE_NAME} (mem_usage) VALUES (?)",
            (usage,),
        )
        conn.commit()
        print(f"[{COMPONENT_ID}] Memory usage {usage}%")
        time.sleep(POLLING_INTERVAL_SECONDS)


def main():
    parser = argparse.ArgumentParser(description='Memory usage daemon')
