...
llm_outputs
Stores text generated by the main LLM processor (id, llm_id, timestamp, content)
db_access_log
Tracks table reads and writes by recording component_id, table_name, and access_type
Configuration
...
LLM Outputs Table
-----------------
Results generated by the main language model are written to the ``llm_outputs`` table.
This table has the columns:

* ``id`` – primary key
* ``llm_id`` – identifier of the processor
* ``timestamp`` – when the row was created (defaults to ``CURRENT_TIMESTAMP``)
